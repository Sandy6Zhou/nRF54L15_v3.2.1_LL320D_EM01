...
static int cmd_lora_test(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (argc != 2)
    {
        shell_print(sh, "Usage: app lora tx");
        return -EINVAL;
    }
    ...
    shell_print(sh, "Usage: app lora tx (sends configured test payload)");
    return -EINVAL;
}
#endif
...
#if MY_LORA_SHELL_TEST_ENABLE
    SHELL_CMD(lora, NULL, "LR1121 test payload: app lora tx", cmd_lora_test),
#endif
#if FS_STORE_TEST_ENABLE
