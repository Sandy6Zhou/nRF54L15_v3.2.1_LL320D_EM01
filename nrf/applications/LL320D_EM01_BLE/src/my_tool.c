...
**入口参数:  无
**出口参数:  无
**函数功能:  初始化 CRC32 计算（标准 CRC-32/ISO-HDLC, 反射算法）
**返 回 值:  CRC32 初始值 0xFFFFFFFF
**注意事项:  配合 my_crc32_update / my_crc32_final 流式增量计算整包校验
*********************************************************************/
uint32_t my_crc32_init(void)
{
    return 0xFFFFFFFFu;
}

/********************************************************************
**函数名称:  my_crc32_update
**入口参数:  crc  --- 当前累计 CRC 值(初始由 my_crc32_init 提供)
**           data --- 本次数据块指针
**           len  --- 本次数据块长度
**出口参数:  无
**函数功能:  增量更新 CRC32（标准 CRC-32/ISO-HDLC, 反射多项式 0xEDB88320）
**返 回 值:  更新后的累计 CRC32 值
**注意事项:  需分块调用时按顺序传入各数据块, 与一次性计算全量等价
*********************************************************************/
uint32_t my_crc32_update(uint32_t crc, const uint8_t *data, uint32_t len)
{
    uint32_t i;
    uint8_t j;
...
/********************************************************************
**函数名称:  my_crc32_final
**入口参数:  crc  --- my_crc32_update 累计后的 CRC 值
**出口参数:  无
**函数功能:  结束 CRC32 计算(输出异或 0xFFFFFFFF), 得到最终校验值
**返 回 值:  最终 CRC32 校验值
**注意事项:  与 zlib / 标准 crc32 一致, 对端可用现成 crc32 库比对
*********************************************************************/
uint32_t my_crc32_final(uint32_t crc)
{
    return ~crc;
}
