...
** 作    者:       曹阳 (caoyang@jimiiot.com)
** 修改内容:        1. 增加产测相关指令。
**                 2. 新增 send_lte_msg，用于 4G 透传数据回传。
**                 3. 气压/温湿度/状态查询按产测模式、LTE 透传来源自动切换 LTE 或 BLE 通道回传。
**                 4. 增加LTE+CMD 透传指令回复。
**                 5. g_lte_cmdSource延时清除，确保温湿度等能回传给4G。
**                 6. 查询指令返回值调整：PATM/TEMP/STATUS 由 BLE_DATA_TYPE_PACKET_MULTIPLE改为 1，改为纯异步回传。
**                 7. 新增 my_sensor_pwr_on，极性改为低电平使能,控制温湿度和气压传感器的电源使能。
**                 8. 修正 SPA06 芯片 ID，由 `0x11` 改为 `0x10`，修复初始化校验失败。
**                 9. 气压/温湿度响应缓冲区扩容，适配新增前缀与产测格式。
***/
